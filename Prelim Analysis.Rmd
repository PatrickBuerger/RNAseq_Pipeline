...
gene_host_coef_sig_lfc2 <- decideTests(gene_host_fit, lfc=2) %>% as.data.frame() %>%  bind_cols(gene_host_fit$genes)

gene_host_coef_sig_lfc1 <- decideTests(gene_host_fit, lfc=1) %>% as.data.frame() %>%  bind_cols(gene_host_fit$genes)

gene_host_coef_sig_all <- decideTests(gene_host_fit) %>% as.data.frame() %>%  bind_cols(gene_host_fit$genes)
...
## CONTRAST COMPARING THE SS8 to the other host types
cont.matrix1_host <- makeContrasts(SS8.vs.WT10 = StrainStrain8 - StrainStrain10, SS8.vs.SS3 = StrainStrain8 - StrainStrain3, SS8.vs.SS5 = StrainStrain8 - StrainStrain5, levels=design)
cont.matrix1_host
## fit contrasts
fit.cont1host <- contrasts.fit(gene_host_fit_NEW, cont.matrix1_host) %>% eBayes()
# fit.cont
# dim(fit.cont1host)
summary(decideTests(fit.cont1host))
# check with other intercept model that up and down regulated genes are still the same between both models - yes, nice..

decideTests(fit.cont1host) %>% 
  vennDiagram(include=c("up","down"),
            counts.col=c("brown3", "darkblue"),
            circle.col = c("darkred", "blue", "green4"))
...
```{r DE-heatmap-plot-host, eval = FALSE}
...
#####
# Filter genes called as DE in SS8 compared to all other strains - host.
any_DE_genes_host <- decideTests(fit.cont1host, lfc=2) %>% 
  as.data.frame() %>% 
  bind_cols(fit.cont1host$genes) %>% 
  filter_if(~ is.numeric(.), any_vars(. != 0))
...
## Heatmap of DE genes
# Normalise expression per-gene
normalised_DE_gene_expression_host <- gene_host_v_NEW[gene_host_v_NEW$genes$gene %in% any_DE_genes_host$gene, ] %>% 
  as.data.frame() %>% 
  gather(SampleName, log2cpm, -gene, -Annotation) %>% 
  left_join(host_samples) %>% 
  group_by(gene) %>%
...
heatmap.data <- fit.cont1symbiont_TopSign
#
...
dds.pcoa3=pcoa(vegdist(t(logcounts),method="euclidean")/1000)
# export values
...
#
ad=adonis(t(logcounts)~CellType+Strain, permutations = 999, data=conditions,method="euclidean")
ad 
capture.output(ad,file=FileName)
#
Title <- paste(SampleGroup,"TukeyHSD")
FileName <- paste("5-",Title,time,".txt")
ad=adonis(t(logcounts)~Strain,data=conditions,method="euclidean")
ad
pco1=scores3[,1]
