...
# Glyoxylate cycle associated genes
fit.cont2host_glyoxy <- topTable(fit.cont1_host, number = Inf, p.value = 0.05) %>% filter(Annotation %>% str_detect("[A|a]cetyl|[G|g]lyoxylate|[M|m]alate|[A|a]conitase|[I|i]socitrate"))
fit.cont2host_glyoxy
...
heatmap.data.host <- fit.cont1host_TopSign
#
...
# Photosynthesis associated genes
fit.cont2symbiont_photo <- topTable(fit.cont1_symbiont, number = Inf, p.value = 0.05) %>% filter(Annotation %>% str_detect("[R|r]ibulose|[R|r]bcL|[P|p]hosphoglycolate phosphatase|[P|p]hosphoglycerate phosphatase| [P|p]hosphoglycerate kinase|[F|f]ructose-bisphosphate"))
fit.cont2symbiont_photo

# Heat map of Top significant genes
fit.cont1symbiont_TopSign <- topTable(fit.cont2_symbiont, number = 500)


####
# Data feed for heat map, change as required..
heatmap.data.symbiont <- fit.cont2symbiont_photo
#
...
#
# Plot it
expression_heatmap_symbiont_photo <- normalised_DE_gene_expression_symbiont %>% 
  mutate(gene = factor(gene, levels = gene %>% unique %>% .[clustering_symbiont$order] )) %>% 
  ggplot(aes(x=SampleName, y = gene, fill = scaled_log2cpm)) +
  geom_tile() +
  scale_fill_gradient2(low = "#0073ff", high = "#ff8c00") +
  ...
  xlab("Sample Strain")
#
expression_heatmap_symbiont_photo
...
# USE THESE FOR INDIVIDUAL GENE COMPARISONS IN REVIGO
write.xlsx(genes_sign_WT10_Intercept_symbiont, file="symbiont_unique_DGE_expression_list.xlsx", sheetName="SS8_vs_WT10_unique", append=TRUE, row.names=FALSE)
#
...
Title <- paste(SampleGroup,"Heat-Map-Symbiont_calvin")
FileName <- paste(Title,time,".jpg")
jpeg(file=FileName, width = 2520, height = 520, res = 200)
par(mfrow=c(1,2))
expression_heatmap_symbiont_photo
dev.off() 

Title <- paste(SampleGroup,"Heat-Map-Host_Topsign500")
FileName <- paste(Title,time,".jpg")
jpeg(file=FileName, width =1520, height = 1020, res = 200)
par(mfrow=c(1,2))
expression_heatmap_host_Topsign
dev.off() 
...
highly_variable_lcpm_NEW <- read.delim("data/GO_Cats_heatmap.txt", row.names=1)
...
color.palette  <- c(colorRampPalette(c("blue", "lightgrey", "red"))(n=20))
...
highly_variable_lcpm_NEW_HOST <- read.delim("data/GO_Cats_heatmap_host.txt", row.names=1)
highly_variable_lcpm_NEWNEW_HOST <- as.matrix(highly_variable_lcpm_NEW_HOST, mode='any')
...
column_labels <- paste(colnames(highly_variable_lcpm_NEW_HOST)) #$CellType, symbiont_samples$Strain, sep = ".")
#col_breaks = c(seq(0.001,0.2,length=100), seq(0.3,0.4,length=100), seq(0.5,0.6,length=100))
#column_labels <- paste(sampleinfo$SampleName)
#
heatmap.2(
  highly_variable_lcpm_NEWNEW_HOST,
  #Rowv = FALSE,
  #Colv = FALSE,
  #col = rev(morecols(9)),
  #col=bluered(256),
