...
gene_host_coef_sig_SS8_interc_lfc2 <- decideTests(gene_host_fit, lfc=2)[,-4] %>% as.data.frame() %>%  bind_cols(gene_host_fit$genes)

gene_host_coef_sig_SS8_interc_lfc1 <- decideTests(gene_host_fit, lfc=1)[,-4] %>% as.data.frame() %>%  bind_cols(gene_host_fit$genes)

gene_host_coef_sig_SS8_interc_all <- decideTests(gene_host_fit)[,-4] %>% as.data.frame() %>%  bind_cols(gene_host_fit$genes)
...
decideTests(gene_host_fit, lfc=1)[,-4] %>% 
  vennDiagram(include=c("up","down"),
            counts.col=c("brown3", "darkblue"),
            circle.col = c("darkred", "blue", "green4"))
title("Venndiagram host. SS8 strain vs all other [lfc > 1]", line = 1)
...
cont.matrix1 <- makeContrasts(SS3.vs.WT10 = StrainStrain3 - StrainStrain10, SS5.vs.WT10 = StrainStrain5 - StrainStrain10, SS8.vs.WT10 = StrainStrain8 - StrainStrain10, SS3.vs.SS5 = StrainStrain3 - StrainStrain5, SS8.vs.SS3 = StrainStrain8 - StrainStrain3, SS8.vs.SS5 = StrainStrain8 - StrainStrain5, levels=design)
cont.matrix1

## fit contrasts
fit.cont <- contrasts.fit(gene_host_fit_NEW, cont.matrix1)
...
```
DATA EXPORT AT THE END OF CODE...
<br><br>
...
gene_symbiont_coef_sig_SS8_interc_lfc1 <- decideTests(gene_symbiont_fit, lfc=1)[,-4] %>% as.data.frame() %>%  bind_cols(gene_symbiont_fit$genes)

gene_symbiont_coef_sig_SS8_interc_lfc0.5 <- decideTests(gene_symbiont_fit, lfc=0.5)[,-4] %>% as.data.frame() %>%  bind_cols(gene_symbiont_fit$genes)

gene_symbiont_coef_sig_SS8_interc_all <- decideTests(gene_symbiont_fit)[,-4] %>% as.data.frame() %>%  bind_cols(gene_symbiont_fit$genes)
...
decideTests(gene_symbiont_fit, lfc=0.5)[,-1] %>% 
  vennDiagram(include=c("up","down"),
            counts.col=c("brown3", "darkblue"),
            circle.col = c("darkred", "blue", "green4"))
title("Venndiagram symbiont. SS strains vs Wild type [lfc > 0.5]", line = 1)

decideTests(gene_symbiont_fit)[,-4] %>% 
  vennDiagram(include=c("up","down"),
            counts.col=c("brown3", "darkblue"),
            circle.col = c("darkred", "blue", "green4"))
title("Venndiagram symbiont. SS8 strain vs all other strains [lfc > 0.5]", line = 1)
...
## CONTRAST COMPARING THE WT and SS TYPEs WITH EACH OTHER
cont.matrix2 <- makeContrasts(SS3.vs.WT10 = StrainStrain3 - StrainStrain10, SS5.vs.WT10 = StrainStrain5 - StrainStrain10, SS8.vs.WT10 = StrainStrain8 - StrainStrain10, SS3.vs.SS5 = StrainStrain3 - StrainStrain5, SS8.vs.SS3 = StrainStrain8 - StrainStrain3, SS8.vs.SS5 = StrainStrain8 - StrainStrain5, levels=design)
cont.matrix2

## fit contrasts
fit.cont <- contrasts.fit(gene_symbiont_fit_NEW, cont.matrix2)
fit.cont <- eBayes(fit.cont)
## check it out
# fit.cont
dim(fit.cont)
...
library(openxlsx)
SampleGroup<-"Symbiont-NoIntercept"
Title <- paste(SampleGroup,"DGE-Table")
FileName <- paste(Title,".xlsx")
limma.res1 <- topTable(fit.cont,coef="SS3.vs.WT10",sort.by="p",n="Inf")
limma.res2 <- topTable(fit.cont,coef="SS5.vs.WT10",sort.by="p",n="Inf")
