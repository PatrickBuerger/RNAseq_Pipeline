...
## Make additional contrasts to compare symbiont SS8 to other symbiont strains (SS3, SS5 and WT10).
...
## fit contrasts
fit.cont1symbiont <- contrasts.fit(gene_symbiont_fit_NEW, cont.matrix_sym) %>% eBayes()
# fit.cont
# dim(fit.cont1symbiont)
summary(decideTests(fit.cont1symbiont))


decideTests(fit.cont1symbiont) %>% 
  vennDiagram(include=c("up","down"),
            counts.col=c("brown3", "darkblue"),
            circle.col = c("darkred", "blue", "green4"))
...
```{r DATA EXPORT Symbiont, eval=FALSE}
symbiont_SS8_VS_all <- topTable(fit.cont1symbiont, n=Inf) %>% write.table(, file="symbiont_SS8_VS_all_rawTable.csv", sep="\t", quote=F)
