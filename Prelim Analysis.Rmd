...
## Make additional contrasts to compare host with SS8 to hosts with other symbiont strains (SS3 and SS5).
```{r Make contrasts Host}

# re-design model without intercept
gene_host_v_NEW <- voom(gene_host_filtered2, design = model.matrix(~0 +Strain, data = gene_host_filtered2$samples), plot = T)

gene_host_fit_NEW <- lmFit(gene_host_v_NEW, design = model.matrix(~0 + Strain, data = gene_host_filtered2$samples)) 

design <- model.matrix(~0 + Strain, data = gene_host_filtered2$samples)

colnames(design)

## CONTRAST COMPARING THE WT and SS TYPEs WITH EACH OTHER
cont.matrix <- makeContrasts(SS3.vs.WT10 = StrainStrain3 - StrainStrain10, SS5.vs.WT10 = StrainStrain5 - StrainStrain10, SS8.vs.WT10 = StrainStrain8 - StrainStrain10, SS3.vs.SS5 = StrainStrain3 - StrainStrain5, SS8.vs.SS3 = StrainStrain8 - StrainStrain3, SS8.vs.SS5 = StrainStrain8 - StrainStrain5, levels=design)
cont.matrix

## fit contrasts
fit.cont <- contrasts.fit(gene_host_fit_NEW, cont.matrix)
fit.cont <- eBayes(fit.cont)
## check it out
# fit.cont
dim(fit.cont)
summary(decideTests(fit.cont))
# check with other intercept model that up and down regulated genes are still the same between both models - yes, nice..

```

```{r Data Export Contrasts Host, eval=FALSE}
library(openxlsx)
SampleGroup<-"Host-NEW"
Title <- paste(SampleGroup,"DGE-Table")
FileName <- paste(Title,".xlsx")
limma.res1 <- topTable(fit.cont,coef="SS3.vs.WT10",sort.by="p",n="Inf")
limma.res2 <- topTable(fit.cont,coef="SS5.vs.WT10",sort.by="p",n="Inf")
limma.res3 <- topTable(fit.cont,coef="SS8.vs.WT10",sort.by="p",n="Inf")
limma.res4 <- topTable(fit.cont,coef="SS3.vs.SS5",sort.by="p",n="Inf")
limma.res5 <- topTable(fit.cont,coef="SS8.vs.SS3",sort.by="p",n="Inf")
limma.res6 <- topTable(fit.cont,coef="SS8.vs.SS5",sort.by="p",n="Inf")
list_of_datasets_host <- list("SS3.vs.WT10" = limma.res1, "SS5.vs.WT10" = limma.res2, "SS8.vs.WT10" = limma.res3, "SS3.vs.SS5" = limma.res4, "SS8.vs.SS3" = limma.res5, "SS8.vs.SS5" = limma.res6)
...
head(topTable(gene_host_fit, number = Inf, p.value = 0.05) %>% 
  filter(Annotation %>% str_detect("[H|h]eat|[G|g]lutathione|[C|c]haperone|[D|d]ismutase|[P|p]eroxidase")))
...
#layout(matrix(c(1,2), 2, 2, byrow = TRUE))
par(mfrow=c(1,2))
plotMDS(gene_symbiont_v, main = "gene counts",labels = gene_symbiont_v$targets$FileName %>% str_remove("symbiont\\."), col = factor( gene_symbiont_v$targets$FileName %>% str_replace("symbiont\\.(.*?)\\..*", "\\1")) %>% as.integer())
#
...
#call genes as DE for different coefficients
#Limiting to a 2X change or greater (1 log2 fold change.. lfc1 = 2 fold change..)
gene_symbiont_coef_sig_lfc1 <- decideTests(gene_symbiont_fit, lfc=1) %>% as.data.frame() %>%  bind_cols(gene_symbiont_fit$genes)

gene_symbiont_coef_sig_lfc0.5 <- decideTests(gene_symbiont_fit, lfc=0.5) %>% as.data.frame() %>%  bind_cols(gene_symbiont_fit$genes)

gene_symbiont_coef_sig_all <- decideTests(gene_symbiont_fit) %>% as.data.frame() %>%  bind_cols(gene_symbiont_fit$genes)
...
decideTests(gene_symbiont_fit)[,-1] %>% 
  vennDiagram(include=c("up","down"),
            counts.col=c("brown3", "darkblue"),
            circle.col = c("darkred", "blue", "green4"))
title("Venndiagram symbiont. SS strains vs Wild type [all genes]", line = 1)
...
## Make additional contrasts to compare symbiont SS8 to other symbiont strains (SS3 and SS5).
```{r Make contrasts Symbiont}

# re-design model without intercept
gene_symbiont_v_NEW <- voom(gene_symbiont_filtered2, design = model.matrix(~0 +Strain, data = gene_symbiont_filtered2$samples), plot = T)

gene_symbiont_fit_NEW <- lmFit(gene_symbiont_v_NEW, design = model.matrix(~0 +Strain, data = gene_symbiont_filtered2$samples)) 
...
## fit contrasts
fit.cont <- contrasts.fit(gene_symbiont_fit_NEW, cont.matrix)
fit.cont <- eBayes(fit.cont)
## check it out
# fit.cont
dim(fit.cont)
...
```{r Data Export Contrasts Symbiont, eval=FALSE}
library(openxlsx)
SampleGroup<-"Symbiont-NEW"
Title <- paste(SampleGroup,"DGE-Table")
FileName <- paste(Title,".xlsx")
limma.res1 <- topTable(fit.cont,coef="SS3.vs.WT10",sort.by="p",n="Inf")
limma.res2 <- topTable(fit.cont,coef="SS5.vs.WT10",sort.by="p",n="Inf")
...
limma.res5 <- topTable(fit.cont,coef="SS8.vs.SS3",sort.by="p",n="Inf")
limma.res6 <- topTable(fit.cont,coef="SS8.vs.SS5",sort.by="p",n="Inf")
list_of_datasets_symbiont <- list("SS3.vs.WT10" = limma.res1, "SS5.vs.WT10" = limma.res2, "SS8.vs.WT10" = limma.res3, "SS3.vs.SS5" = limma.res4, "SS8.vs.SS3" = limma.res5, "SS8.vs.SS5" = limma.res6)
...
Filter genes from Venndiagram. Which are the genes that all selected strains have significantly expressed compared to the wild type?
```{r DE-heatmap-plot-host}
# Filter genes called as DE to control in any strain
any_DE_genes_symbiont <- decideTests(gene_symbiont_fit, lfc=1)[,-1] %>% 
  as.data.frame() %>% 
  bind_cols(gene_symbiont_fit$genes) %>% 
  filter_if(~ is.numeric(.), any_vars(. != 0))
...
## Heatmap of DE genes
# Normalise expression per-gene
normalised_DE_gene_expression_symbiont <- gene_symbiont_v[gene_symbiont_v$genes$gene %in% any_DE_genes_symbiont$gene, ] %>% 
  as.data.frame() %>% 
  gather(SampleName, log2cpm, -gene, -Annotation) %>% 
...
symbiont_strain_8_up_all <- filter(gene_symbiont_coef_sig_all, StrainStrain8 == 1) %>% .[order(.$Annotation),]
...
symbiont_strain_8_down_all <- filter(gene_symbiont_coef_sig_all, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
...
#
symbiont_strain_8_unique_up_0.5 <- filter(gene_symbiont_coef_sig_0.5, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
symbiont_strain_8_unique_down_0.5 <- filter(gene_symbiont_coef_sig_0.5, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
symbiont_strain_8_unique_up_all <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
symbiont_strain_8_unique_down_all <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
...
#
write.xlsx(symbiont_strain_8_up_all, file="symbiont_DGE_expression_listNEW.xlsx", sheetName="symbiont_strain_8_up_all", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strain_8_down_all, file="symbiont_DGE_expression_listNEW.xlsx", sheetName="symbiont_strain_8_down_all", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strain_8_unique_up_0.5, file="symbiont_DGE_expression_listNEW.xlsx", sheetName="symbiont_strain_8_unique_up_0.5", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strain_8_unique_down_0.5, file="symbiont_DGE_expression_listNEW.xlsx", sheetName="symbiont_strain_8_unique_down_0.5", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strain_8_unique_up_all, file="symbiont_DGE_expression_listNEW.xlsx", sheetName="symbiont_strain_8_unique_up_all", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strain_8_unique_down_all, file="symbiont_DGE_expression_listNEW.xlsx", sheetName="symbiont_strain_8_unique_down_all", append=TRUE, row.names=FALSE)
...
#plot_symbiont_gene("SymbC1.scaffold10700.1.m1")
...
* Annotate with GO categories for functional analysis
    - DONE, wouldbe good to have a different contrast, SS8 vs SS3+SS5
...
```{r, eval=FALSE, echo=FALSE}
#
symbiont_strains_3_5_8_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 1, StrainStrain5 == 1, StrainStrain8 == 1) %>% .[order(.$Annotation),]

symbiont_strains_3_5_8_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == -1, StrainStrain5 == -1, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
lfc1_SS_3_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 1) %>% .[order(.$Annotation),] 
lfc1_SS_5_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain5 == 1)%>% .[order(.$Annotation),]
lfc1_SS_8_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain8 == 1) %>% .[order(.$Annotation),] 
lfc1_SS_3_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == -1) %>% .[order(.$Annotation),]
lfc1_SS_5_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain5 == -1) %>% .[order(.$Annotation),] 
lfc1_SS_8_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain8 == -1) %>% .[order(.$Annotation),] 
#
lfc1_SS_3_unique_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 1, StrainStrain5 == 0, StrainStrain8 == 0) %>% .[order(.$Annotation),]
lfc1_SS_3_unique_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == -1, StrainStrain5 == 0, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
lfc1_SS_5_unique_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 0, StrainStrain5 == 1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
lfc1_SS_5_unique_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 0, StrainStrain5 == -1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
lfc1_SS_8_unique_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
lfc1_SS_8_unique_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
lfc1_SS_3_8_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 1, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
lfc1_SS_3_8_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == -1, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),] 
#
lfc1_SS_3_5_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 1, StrainStrain5 == 1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
lfc1_SS_3_5_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == -1, StrainStrain5 == -1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
lfc1_SS_5_8_up <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 0, StrainStrain5 == 1, StrainStrain8 == 1) %>% .[order(.$Annotation),]
lfc1_SS_5_8_down <- filter(gene_symbiont_coef_sig_lfc1, StrainStrain3 == 0, StrainStrain5 == -1, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
library(xlsx)
#install.packages("xlsx")
write.xlsx(symbiont_strains_3_5_8_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="symbiont_strains_3_5_8_up", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strains_3_5_8_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="symbiont_strains_3_5_8_down", append=TRUE, row.names=FALSE)
#
write.xlsx(lfc1_SS_3_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_3_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_3_unique_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_3_unique_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_unique_down", append=TRUE, row.names=FALSE)
#
write.xlsx(lfc1_SS_5_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_5_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_5_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_5_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_5_unique_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_5_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_5_unique_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_5_unique_down", append=TRUE, row.names=FALSE)
#
write.xlsx(lfc1_SS_8_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_8_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_8_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_8_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_8_unique_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_8_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_8_unique_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_8_unique_down", append=TRUE, row.names=FALSE)
#
#
write.xlsx(lfc1_SS_3_8_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_8_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_3_8_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_8_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_3_5_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_5_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_3_5_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_3_5_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_5_8_up, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_5_8_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc1_SS_5_8_down, file="lfc1_symbiont_DGE_expression_list.xlsx", sheetName="lfc1_SS_5_8_down", append=TRUE, row.names=FALSE)
#
#
symbiont_strains_3_5_8_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 1, StrainStrain5 == 1, StrainStrain8 == 1) %>% .[order(.$Annotation),]
symbiont_strains_3_5_8_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == -1, StrainStrain5 == -1, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
lfc0.5_SS_3_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 1) %>% .[order(.$Annotation),] 
lfc0.5_SS_5_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain5 == 1)%>% .[order(.$Annotation),]
lfc0.5_SS_8_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain8 == 1) %>% .[order(.$Annotation),] 
lfc0.5_SS_3_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == -1) %>% .[order(.$Annotation),]
lfc0.5_SS_5_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain5 == -1) %>% .[order(.$Annotation),] 
lfc0.5_SS_8_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain8 == -1) %>% .[order(.$Annotation),] 
#
lfc0.5_SS_3_unique_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 1, StrainStrain5 == 0, StrainStrain8 == 0) %>% .[order(.$Annotation),]
lfc0.5_SS_3_unique_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == -1, StrainStrain5 == 0, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
lfc0.5_SS_5_unique_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 0, StrainStrain5 == 1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
lfc0.5_SS_5_unique_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 0, StrainStrain5 == -1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
lfc0.5_SS_8_unique_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
lfc0.5_SS_8_unique_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
lfc0.5_SS_3_8_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 1, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
lfc0.5_SS_3_8_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == -1, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),] 
#
lfc0.5_SS_3_5_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 1, StrainStrain5 == 1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
lfc0.5_SS_3_5_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == -1, StrainStrain5 == -1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
lfc0.5_SS_5_8_up <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 0, StrainStrain5 == 1, StrainStrain8 == 1) %>% .[order(.$Annotation),]
lfc0.5_SS_5_8_down <- filter(gene_symbiont_coef_sig_lfc0.5, StrainStrain3 == 0, StrainStrain5 == -1, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
library(xlsx)
#install.packages("xlsx")
write.xlsx(symbiont_strains_3_5_8_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="symbiont_strains_3_5_8_up", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strains_3_5_8_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="symbiont_strains_3_5_8_down", append=TRUE, row.names=FALSE)
#
write.xlsx(lfc0.5_SS_3_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_3_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_3_unique_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_3_unique_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_unique_down", append=TRUE, row.names=FALSE)
#
write.xlsx(lfc0.5_SS_5_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_5_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_5_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_5_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_5_unique_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_5_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_5_unique_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_5_unique_down", append=TRUE, row.names=FALSE)
#
write.xlsx(lfc0.5_SS_8_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_8_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_8_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_8_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_8_unique_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_8_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_8_unique_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_8_unique_down", append=TRUE, row.names=FALSE)
#
#
write.xlsx(lfc0.5_SS_3_8_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_8_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_3_8_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_8_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_3_5_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_5_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_3_5_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_3_5_down", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_5_8_up, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_5_8_up", append=TRUE, row.names=FALSE)
write.xlsx(lfc0.5_SS_5_8_down, file="lfc0.5_symbiont_DGE_expression_list.xlsx", sheetName="lfc0.5_SS_5_8_down", append=TRUE, row.names=FALSE)
#
#
symbiont_strains_3_5_8_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 1, StrainStrain5 == 1, StrainStrain8 == 1) %>% .[order(.$Annotation),]
symbiont_strains_3_5_8_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == -1, StrainStrain5 == -1, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
all_SS_3_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 1) %>% .[order(.$Annotation),] 
all_SS_5_up <- filter(gene_symbiont_coef_sig_all, StrainStrain5 == 1)%>% .[order(.$Annotation),]
all_SS_8_up <- filter(gene_symbiont_coef_sig_all, StrainStrain8 == 1) %>% .[order(.$Annotation),] 
all_SS_3_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == -1) %>% .[order(.$Annotation),]
all_SS_5_down <- filter(gene_symbiont_coef_sig_all, StrainStrain5 == -1) %>% .[order(.$Annotation),] 
all_SS_8_down <- filter(gene_symbiont_coef_sig_all, StrainStrain8 == -1) %>% .[order(.$Annotation),] 
#
all_SS_3_unique_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 1, StrainStrain5 == 0, StrainStrain8 == 0) %>% .[order(.$Annotation),]
all_SS_3_unique_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == -1, StrainStrain5 == 0, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
all_SS_5_unique_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == 1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
all_SS_5_unique_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == -1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
all_SS_8_unique_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
all_SS_8_unique_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
all_SS_3_8_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 1, StrainStrain5 == 0, StrainStrain8 == 1) %>% .[order(.$Annotation),]
all_SS_3_8_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == -1, StrainStrain5 == 0, StrainStrain8 == -1) %>% .[order(.$Annotation),] 
#
all_SS_3_5_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 1, StrainStrain5 == 1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
all_SS_3_5_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == -1, StrainStrain5 == -1, StrainStrain8 == 0) %>% .[order(.$Annotation),]
#
all_SS_5_8_up <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == 1, StrainStrain8 == 1) %>% .[order(.$Annotation),]
all_SS_5_8_down <- filter(gene_symbiont_coef_sig_all, StrainStrain3 == 0, StrainStrain5 == -1, StrainStrain8 == -1) %>% .[order(.$Annotation),]
#
library(xlsx)
#install.packages("xlsx")
write.xlsx(symbiont_strains_3_5_8_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="symbiont_strains_3_5_8_up", append=TRUE, row.names=FALSE)
write.xlsx(symbiont_strains_3_5_8_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="symbiont_strains_3_5_8_down", append=TRUE, row.names=FALSE)
#
write.xlsx(all_SS_3_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_3_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_down", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_3_unique_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_3_unique_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_unique_down", append=TRUE, row.names=FALSE)
#
write.xlsx(all_SS_5_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_5_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_5_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_5_down", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_5_unique_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_5_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_5_unique_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_5_unique_down", append=TRUE, row.names=FALSE)
#
write.xlsx(all_SS_8_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_8_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_8_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_8_down", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_8_unique_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_8_unique_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_8_unique_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_8_unique_down", append=TRUE, row.names=FALSE)
#
#
write.xlsx(all_SS_3_8_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_8_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_3_8_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_8_down", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_3_5_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_5_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_3_5_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_3_5_down", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_5_8_up, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_5_8_up", append=TRUE, row.names=FALSE)
write.xlsx(all_SS_5_8_down, file="all_symbiont_DGE_expression_list.xlsx", sheetName="all_SS_5_8_down", append=TRUE, row.names=FALSE)
